...
interface RetryRingProps {
  ...
  tone: "accent" | "danger";
  children: ReactNode;
  // Off for a state that will not resolve on its own (see TerminalView's non-recoverable
  // DisconnectedOverlay variant): a spinning ring implies "in progress", which is misleading
  // once auto-retry has actually stopped.
  spinning?: boolean;
}
...
export function RetryRing({ size, tone, children, spinning = true }: RetryRingProps) {
